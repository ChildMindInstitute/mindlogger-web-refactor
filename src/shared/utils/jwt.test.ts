...
describe('parseJwtClaims', () => {
  it('returns every claim in the payload', () => {
  ...
  it.each([
    ...
    ['a payload that is not JSON', `header.${window.btoa('plain text')}.signature`],
    ['a payload that is not an object', `header.${window.btoa('42')}.signature`],
...
describe('getTokenExpiration', () => {
  ...
  it('is comparable against Date.now()', () => {
