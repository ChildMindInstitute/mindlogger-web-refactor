...
describe('sessionSync', () => {
  ...
  it('delivers a published message to another tab', () => {
  ...
  it('stays silent while nothing in this tab is listening', () => {
    const sibling = openSiblingTab();
    const onSiblingMessage = vi.fn();
    sibling.onmessage = onSiblingMessage;
  ...
  it('does not deliver a published message back to the publisher', () => {
    const handler = vi.fn();
  ...
  it('delivers a message from another tab to every subscriber', () => {
    const first = vi.fn();
    const second = vi.fn();
  ...
  it('ignores a payload that is not a session message', () => {
    const handler = vi.fn();
    subscribeSessionSync(handler);
