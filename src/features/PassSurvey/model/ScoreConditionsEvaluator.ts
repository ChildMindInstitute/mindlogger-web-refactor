import { ScoreConditionalLogic } from '~/shared/api';
...
export class ScoreConditionsEvaluator implements IScoreConditionsEvaluator {
  private evaluateInternal(settings: ScoreConditionalLogic, score: number): boolean {
    const expressionResults: Array<boolean> = settings.conditions.map((condition) => {
      switch (condition.type) {
        case 'BETWEEN':
          return condition.payload.minValue < score && score < condition.payload.maxValue;
        case 'EQUAL':
        ...
        case 'NOT_EQUAL':
          return score !== condition.payload.value;
        case 'OUTSIDE_OF':
          return score < condition.payload.minValue || score > condition.payload.maxValue;
  ...
  public evaluate(settings: ScoreConditionalLogic, score: number): boolean {
    try {
      ...
      throw new Error(`[ScoreConditionsEvaluator.evaluate]: Error occurred:\n\n${error}`);
    }
  }
}
