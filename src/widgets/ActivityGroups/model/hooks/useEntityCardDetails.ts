...
export const useEntityCardDetails = (props: Props): Return => {
  const isFlow = Boolean(
    props.activityListItem.flowId && props.activityListItem.activityFlowDetails,
  );

  const flow = props.applet.activityFlows.find((flow) => flow.id === props.activityListItem.flowId);

  const showActivityFlowBudget = Boolean(
    props.activityListItem.activityFlowDetails?.showActivityFlowBadge,
  );
  ...
  const activity =
    props.applet.activities.find((activity) => activity.id === props.activityListItem.activityId) ??
    null;

  const activitiesInFlow = props.applet.activities.filter(({ id }) =>
    flow?.activityIds.includes(id),
  );
