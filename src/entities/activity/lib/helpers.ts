 ...
 */
export const hasAdditionalResponse = (
  item: Pick<appletModel.ItemRecord, 'responseType' | 'config'>,
): boolean => {
  return (
    supportsAdditionalResponseField(item) && item.config.additionalResponseOption.textInputOption
  );
};
 ...
 */
export const requiresAdditionalResponse = (
  item: Pick<appletModel.ItemRecord, 'responseType' | 'config'>,
): boolean => {
  return (
    supportsAdditionalResponseField(item) && item.config.additionalResponseOption.textInputRequired
  );
};
