...
const appletsSlice = createSlice({
  ...
  reducers: {
    ...
    flowRestarted: (state, { payload }: PayloadAction<FlowRestartedPayload>) => {
      ...
      console.info(
        `[DEBUG-FLOW] flowRestarted reducer\n` +
          `  progressId=${id}\n` +
          `  payload.flowId=${payload.flowId}\n` +
          `  payload.activityId=${payload.activityId}\n` +
          `  payload.appletVersion=${payload.appletVersion}\n` +
          `  payload.flowActivityIds=${JSON.stringify(payload.flowActivityIds)}\n` +
          `  payload.flowActivityIds.length=${payload.flowActivityIds.length}\n` +
          `  payload.flowName=${payload.flowName}\n` +
          `  existing submitId=${groupProgress?.submitId ?? 'none'}\n` +
          `  existing appletVersion=${groupProgress?.appletVersion ?? 'none'}\n` +
          `  existing flowActivityIds=${JSON.stringify(groupProgress?.flowActivityIds ?? null)}\n` +
          `  new submitId will be generated by uuidV4`,
      );
