// postDataToEndpoint({ url: fullURL, data: myData, method: 'POST' })
//         .then(() => {
...
import { apiAdminLogin } from './loginPage';
...
export class AppletAPI {
    ...
    async createApplet(appletData: createAppletPayload): Promise<any> {
        const response = await this.request.post('/applets', { data: appletData });
    ...
    async createManagerInvite(inviteData: { email: string; firstName: string; lastName: string; language: string; role: string; workspacePrefix: string; title: string; }, appletID: string): Promise<any> {
        const response = await this.request.post(`/invitations/${appletID}/managers`, { data: inviteData });
        console.log(response);
        expect(response.ok()).toBeTruthy();
        return await response.json();
    }
    ...
    async getAppletInvitations(appletID: string): Promise<any> {
        const response = await this.request.get(`/invitations?page=1&limit=10&ordering=-id&appletId=${appletID}`);
        console.log(response);
        expect(response.ok()).toBeTruthy();
        return await response.json();
    }
